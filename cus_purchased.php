...
<html>
<head>
    <title><?php echo $cusname."'s " ?> Purchases - Car Point</title>
...
$purchasedcarsquery = "SELECT car.carid AS carid, name, DATE_FORMAT(car.paymentdate, '%d %M %Y') AS paymentdate,price, discount 
                      FROM car 
                      INNER JOIN newcar ON car.carid = newcar.carid 
                      WHERE customerid = $cusid AND newcar.paymentstatus = 'verified' 
                      ORDER BY car.paymentdate DESC";
...
where customerid = $cusid and preownedcar.paymentstatus = 'verified' and car.carid = preownedcarid order by paymentdate desc";
