...
// Ensure $carid is an integer to prevent SQL injection
$query1 = "SELECT 
    car.Name, 
    dealer.dName AS Dname, 
    manufacturer.Mname AS Mname, 
    car.status, 
    manufacturer.phoneno AS mph, 
    manufacturer.location AS mloc, 
    manufacturer.email AS memail, 
    manufacturer.website AS mweb, 
    dealer.phoneno AS dph, 
    dealer.d_email AS d_email, 
    dealer.website AS dweb, 
    car.mileage, 
    car.color, 
    car.status, 
    car.fueltype, 
    car.licenseplateno, 
    preownedcar.resaleprice, 
    preownedcar.Mileage, 
    preownedcar.discount, 
    preownedcar.customerid 
FROM 
    car 
INNER JOIN 
    preownedcar ON car.carid = preownedcar.carid 
INNER JOIN 
    manufacturer ON car.manufacturerid = manufacturer.manufacturerid 
INNER JOIN 
    owns ON owns.carid = car.carid 
INNER JOIN 
    dealer ON owns.dealerid = dealer.dealerid 
WHERE 
    car.carid = $carid";
...
<html>
<head>
    <title><?php echo $firstquery["Name"]?> - Car Point</title>
...
<div class="tab-content" id="nav-tabContent">
  ...
  <div class="card-body">
    ...
    <p class="card-text"><b>Kilometers driven - </b><?php echo $firstquery["mileage"]?></p>
