...
<html>
<head>
    <title><?php echo $dealername."'s " ?> Sales - Car Point</title>
...
$soldcarsquery = "SELECT 
customer.customername,
car.carid AS carid,
car.name,
DATE_FORMAT(car.paymentdate, '%d %M %Y') AS paymentdate 
FROM 
owns 
INNER JOIN 
customer ON customer.customerid = owns.customerid
INNER JOIN 
car ON car.carid = owns.carid
INNER JOIN 
newcar ON car.carid = newcar.newcarid 
WHERE 
dealerid = $dealerid 
AND newcar.paymentstatus = 'verified' 
ORDER BY 
car.paymentdate DESC;
";
...
car ON car.carid = owns.carid
INNER JOIN 
preownedcar ON car.carid = preownedcar.preownedcarid 
WHERE 
dealerid = $dealerid 
AND preownedcar.paymentstatus = 'verified' 
ORDER BY 
car.paymentdate DESC;
