...
$checkifalreadyrented = "select * from rentalcar where carid=$carid and enddate is null";
$checkex = mysqli_query($conn, $checkifalreadyrented);

if(mysqli_num_rows($checkex)===0)
{
//query to insert into rent table
$setrent = "insert into rentalcar(rentalcarid,customerid,startdate,rentdate) values ($carid,$cusid,STR_TO_DATE('$date', '%d-%c-%Y'),CURRENT_TIMESTAMP())";
...
else
{
    ...
    $endrent = "update rent set enddate=CURRENT_TIMESTAMP() where carid=$carid and customerid=$cusid and startdate='$startdate'";
